...
public class pendingAccounts extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Admin/33.JPG"))); // NOI18N
    ...
    private void displayData() {
        try {
            ResultSet rs = new DBConnector().getData("select id,email,user,type from yang where status = 'PENDING'");
    ...
    private void acceptAccount() {
        ...
        if (rowIndex < 0) {
            ...
            try {
                ...
                new DBConnector().updateData("UPDATE yang SET status = 'ACTIVE' WHERE id = '" + tbl.getValueAt(rowIndex, 0).toString() + "'");
    ...
    private void declineAccount() {
        ...
        if (rowIndex < 0) {
            ...
            try {
                TableModel tbl = pendings.getModel();
                new DBConnector().updateData("UPDATE yang SET status = 'DECLINED' WHERE id = '" + tbl.getValueAt(rowIndex, 0).toString() + "'");
